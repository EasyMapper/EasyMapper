...
class TypeAnalyzer {
    ...
    public static Function<Getter, Type> getReturnTypeResolver(Type type) {
        if (type instanceof Class<?>) {
            ...
            throw new RuntimeException(
                "Cannot support return type resolver for the type: " + type
            );
        }
    }

    private static Function<Getter, Type> getReturnTypeResolver(
    ...
    private static TypeVariable<?>[] getTypeParameters(ParameterizedType type) {
        if (type.getRawType() instanceof Class<?>) {
            ...
            throw new RuntimeException(
                "Cannot provide type parameters for the type: " + type
            );
        }
    }
}
